...
def reWindow(last_window, current_user, player, dealer, action, bet):
    view.role_window.closeWindow(last_window)

    player_hand, dealer_hand, result = game.game.checkDealer(action, player, dealer)
    player_value = game.game.getValue(player_hand)
    dealer_value = game.game.getValue(dealer_hand)
...
def playWindow(last_window, current_user, bet):
    view.role_window.closeWindow(last_window)
    ...
    Button(app, text='Hit',command=lambda: reWindow(app, current_user, player_hand, dealer_hand,1, bet)).grid(row=3,column=0)
    Button(app, text='Stay',command=lambda: reWindow(app, current_user, player_hand, dealer_hand,2, bet)).grid(row=3,column=1)
